...
class Booking(db.Model):
    ...
    def __init__(self, booking_id, tutor_id, tutee_id, payment, subjects, status):
...
def get_all_bookings_for_tutor(tutor_id):
    bookings = Booking.query.filter_by(tutor_id=tutor_id)
    if bookings:
...
def get_all_bookings_for_tutee(tutee_id):
    bookings = Booking.query.filter_by(tutee_id=tutee_id)
    if bookings:
